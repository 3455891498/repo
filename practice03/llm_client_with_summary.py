...
def load_env():
    ...
    从项目根目录的.env文件中读取环境变量，支持BASE_URL、MODEL、API_KEY等配置项。
    默认使用本地Ollama服务配置。
...
def build_tools_system_prompt():
    ...
    根据TOOLS字典生成包含所有工具描述的系统提示词，指导LLM正确生成工具调用。
...
def summarize_chat_history(history, env_vars):
    ...
    当聊天历史过长时，自动触发总结，将历史记录压缩为总结+最近部分。
    
    参数:
        history (list): 聊天历史记录列表
        env_vars (dict): 环境变量配置
    ...
    try:
        # 发送请求
        req = urllib.request.Request(env_vars.get('BASE_URL', 'http://localhost:11434/api/chat'), 
...
def call_llm_stream(prompt, env_vars, history, tools_enabled=True):
    ...
    发送HTTP请求到指定的LLM API端点，以流式方式接收响应并实时输出。
    ...
    返回:
        dict: 包含响应内容、token使用情况、耗时信息和更新后的历史记录的字典
    """
...
def parse_tool_call(content):
    ...
    返回:
        dict: 工具调用字典（包含tool_name和parameters），解析失败返回None
    """
    try:
        # 查找JSON格式的工具调用
        lines = content.split('\n')
...
def main():
    ...
    try:
        while True:
            ...
            if len(history) >= 10 or history_length >= 3000:
